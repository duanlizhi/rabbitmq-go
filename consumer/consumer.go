...
//amqpURI rabbitmq url
//config：带有部分配置参数的消费者
func NewIConsumerByConfig(amqpURI string, durable, autoDelete bool, config rabbitmq_go.IConfig) *Consumer {
	return NewIConsumerByConfigAndDurableAndAutoDelete(amqpURI, true, true, config)
}
...
func NewIConsumerByConfigAndDurableAndAutoDelete(amqpURI string, durable, autoDelete bool,
	config rabbitmq_go.IConfig) *Consumer {
	if amqpURI == "" {
		panic("Cannot initialize connection to broker, please check your profile. Have you initialized?")
	}
...
//autoAck：是否自动ack
//handlerFunc func(delivery amqp.Delivery,autoAck bool)：订阅消息后的业务逻辑处理
func (c *Consumer) SubscribeToQueue(queueName string, consumerTag string, autoAck bool,
	handlerFunc func(delivery amqp.Delivery)) error {
	ch, err := c.conn.Channel()
	if err != nil {
